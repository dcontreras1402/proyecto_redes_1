...
async function obtenerProductosActivos() {
    const [rows] = await db.query('SELECT * FROM productos WHERE activo = true AND aprobado = true');
    return rows;
}
...
async function crearProducto(id_vendedor, nombre, descripcion, precio, cantidad) {
    const [result] = await db.query(
        'INSERT INTO productos (id_vendedor, nombre, descripcion, precio, cantidad, activo, aprobado) VALUES (?, ?, ?, ?, ?, true, false)',
        [id_vendedor, nombre, descripcion, precio, cantidad]
    );
    return result;
}

async function editarProducto(id, id_vendedor, nombre, descripcion, precio, cantidad) {
    const [producto] = await db.query('SELECT id_vendedor FROM productos WHERE id = ?', [id]);
...
async function aprobarProducto(id) {
    const [result] = await db.query('UPDATE productos SET aprobado = true WHERE id = ?', [id]);
    return result;
}
